...
import {  deleteFloodMessage, setFloodMessage, updateFloodMessage } from '../../../redux/actions/usersActions';
import { ChatProps, IMessage, RootState } from '../../../shared/interfaces';

import './ChatFlood.scss';

const ChatFlood: React.FC<ChatProps> = ({ author }) => {
  const messages = useSelector((state: RootState) => state.floodMessages);
  ...
  const submitHandler = () => {
    if (mode === "send") {
      if (message.message) dispatch(setFloodMessage({ ...message, id: Math.ceil(Math.random() * 100000) }));
      setMessage({ message: "", author: author.username, id: 0 });
    } else if (mode === "edit") {
      if (message) dispatch(updateFloodMessage(message));
      setMessage({ message: "", author: author.username, id: 0 });
      setMode("send");
    }
  };
  ...
  const handleDelete = (messageItem: IMessage) => {
    dispatch(deleteFloodMessage(messageItem));
  };
